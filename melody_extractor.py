...
class MxlMelodyExtractor:
    ...
    def __init__(self, fl_nm, precision=5):
        ...
        # ic(list(parts_drum))
        # [self.scr.parts.remove(p) for p in parts_drum
        for p in parts_drum:
            # ic(p)
            self.scr.remove(p)
        for p in self.scr.parts:
        ...
        # for p in self.scr.parts:
        #     for b in list(p[m21.stream.Measure])[:5]:
        #         ic(b)
        #         ic(list(b))
    ...
    class VerticalBar:
        ...
        def pnm_with_max_pitch(self, method='fqs', val_rest=0):
            ...
            for pnm, b in self.bars.items():
                ...
                # if b[m21.note.Unpitched]:
                #     ic(pnm, b.number, list(b))
